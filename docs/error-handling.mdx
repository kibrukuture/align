...
The Align SDK provides a robust error handling system with typed errors that help you identify and handle issues effectively.
...
class AlignValidationError extends Error {
  readonly errors: ValidationError[]; // Array of validation issues
}
...
<AccordionGroup>
  <Accordion title="Duplicate Email (409 Conflict)" icon="clone">
    ...
    try {
      ...
      if (error instanceof AlignError && error.statusCode === 409) {
  ...
  <Accordion title="Invalid API Key (401 Unauthorized)" icon="key">
    ...
    try {
      ...
      if (error instanceof AlignError && error.statusCode === 401) {
  ...
  <Accordion title="Validation Errors" icon="check">
    ...
    try {
      ...
      if (error instanceof AlignValidationError) {
        ...
        //   { path: ["last_name"], message: "Required" }
        // ]
      }
    }
    ```
  ...
  <Accordion title="Rate Limiting (429 Too Many Requests)" icon="gauge-high">
    ...
    try {
      ...
      if (error instanceof AlignError && error.statusCode === 429) {
        // Wait and retry
...
type ErrorResult =
  | { type: "validation"; errors: Array<{ path: string[]; message: string }> }
