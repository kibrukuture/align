...
Use this method to poll for KYC approval status:

```typescript
async function waitForKycApproval(
  customerId: string,
  maxAttempts = 20
): Promise<boolean> {
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (customer.kycs?.sub_status === "kyc_form_submission_accepted") {
      console.log("✅ KYC approved!");
      return true;
    }

    if (customer.kycs?.sub_status === "kyc_form_submission_rejected") {
  ...
  console.log("⏱️ Timeout waiting for KYC");
  return false;
}
```
