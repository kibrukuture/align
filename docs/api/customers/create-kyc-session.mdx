...
Generate a unique KYC (Know Your Customer) verification link that customers use to complete their identity verification process.
...
```json
{
  "kycs": {
    ...
    "status": "pending",
    "sub_status": "kyc_form_submission_pending"
  }
}
```
...
| ---------- | --------------------------------- | --------------------------- |
| `pending`  | `kyc_form_submission_pending`     | Waiting for user to submit  |
| `pending`  | `kyc_form_submission_in_progress` | User is completing the form |
| `pending`  | `under_review`                    | Submitted, being reviewed   |
| `approved` | `kyc_form_submission_accepted`    | Verification successful     |
| `rejected` | `kyc_form_submission_rejected`    | Verification failed         |
