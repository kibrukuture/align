...
import { TRANSFER_PURPOSES } from "./transfers.constants";

export const CreateOfframpQuoteSchema = z
  .object({
    source_amount: z
      .string()
      .regex(/^[0-9]+(\.[0-9]+)?$/)
    ...
    destination_payment_rails: z.enum([
      "ach",
      "wire",
      "sepa",
      "swift",
...
export const CreateOnrampQuoteSchema = z
  .object({
    source_amount: z
      .string()
      .regex(/^[0-9]+(\.[0-9]+)?$/)
...
export const CreateOfframpTransferSchema = z
  .object({
    transfer_purpose: z.enum(TRANSFER_PURPOSES),
    destination_external_account_id: z.uuid().optional(),
    destination_bank_account: z
      .union([IbanAccountDetailsSchema, UsAccountDetailsSchema])
      .optional(),
  })
  .refine(
    (data) =>
      !!data.destination_external_account_id !==
      !!data.destination_bank_account,
    {
      message:
        "Either destination_external_account_id OR destination_bank_account must be provided, but not both",
    }
  );
