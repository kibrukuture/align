...
- **Webhooks**: Added and exported `CustomerKycsUpdatedEventPayload` for the `customer.kycs.updated` webhook body shape
...
- **Webhooks**: `verifySignature(payload, signature)` now defaults to the SDK instance API key when the secret is omitted
  - Still supports `verifySignature(payload, signature, secret)` for overrides
