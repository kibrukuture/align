...
- **Zod Deprecations**:
  ...
  - Implemented `formatZodError` utility for consistent error formatting across all resources
- Added client-side validation for Developer Fees API
...
- **Wallet Ownership API**:
  ...
  - Removed `status` field from response (not in official docs)
...
- **Customers API**:
  ...
  - Removed `'swift'` from `KycStatusBreakdown.payment_rails` enum (not in official docs)
...
- **Webhooks API**:
  ...
  - `GET /webhooks` → `GET /v0/webhooks`
  - `DELETE /webhooks/{id}` → `DELETE /v0/webhooks/{id}`
